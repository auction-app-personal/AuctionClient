...
const MOCK_ACCOUNTS_STORAGE_KEY = "MockAccounts";

@Injectable({
  providedIn: 'root',
})
export class MockAccountService extends LocalStorageService<AccountDto> implements AccountService {
  ...
  registerUser(username: string, password: string): number {
    if(this.items.filter(acc => acc.username === username).length !== 0)
  ...
  loginUser(username: string, password: string): { user: AccountDto | null; token: string; } {
    ...
    const user = this.items.filter(acc => acc.username === username).at(0);
    if(!user) 
      return {user: null, token: ''}; //can't login
